...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DICT_ZH = {
      ...
      previewDialog: '图片预览', previewEdit: '在图像工坊中编辑', previewClose: '关闭预览',
      viewOriginal: '查看原图', thumbRetry: '加载失败，点击重试', imgAlt: '图片',
    ...
    const DICT_EN = {
      ...
      previewDialog: 'Image preview', previewEdit: 'Edit in Image Studio', previewClose: 'Close preview',
      viewOriginal: 'View original', thumbRetry: 'Failed to load — click to retry', imgAlt: 'Image',
    ...
    /** Display box for one lone image: long edge 240px with the rendered
     * aspect ratio clamped to [0.25, 4] and no upscale past natural size —
     * the same rule the shipped gallery applies. */
    function singleFitStyle(dims) {
      if (dims === undefined || dims.width === undefined || dims.height === undefined
        || dims.width <= 0 || dims.height <= 0) {
        return { width: '240px', height: '160px', objectFit: 'cover' };
      }
      const natural = dims.width / dims.height;
      const ratio = Math.min(4, Math.max(0.25, natural));
      const box = ratio >= 1 ? { w: 240, h: 240 / ratio } : { w: 240 * ratio, h: 240 };
      const scale = Math.min(1, dims.width / box.w, dims.height / box.h);
      return {
        width: Math.max(1, Math.round(box.w * scale)) + 'px',
        height: Math.max(1, Math.round(box.h * scale)) + 'px',
        objectFit: 'cover',
    ...
    const CSS = [
      ...
      '.isx-viewport.isx-pannable{cursor:grab;}',
      '.isx-viewport.isx-pannable:active{cursor:grabbing;}',
      '.isx-viewport.isx-panning{cursor:grabbing;user-select:none;}',
      ...
      '.isx-preview{display:flex;flex-wrap:wrap;gap:8px;align-items:flex-start;}',
      '.isx-thumbbtn{border:0;padding:0;background:none;cursor:pointer;border-radius:8px;overflow:hidden;display:block;}',
      '.isx-thumbbtn img{display:block;}',
      '.isx-thumbload{width:64px;height:64px;display:inline-flex;align-items:center;justify-content:center;border:0;',
      'background:var(--dsw-alias-interactive-bg-active,rgba(0,0,0,.06));border-radius:8px;color:var(--dsw-alias-label-tertiary,#7c7f87);',
      'font:inherit;font-size:11px;cursor:pointer;padding:0 6px;text-align:center;}',
      '.isx-lb{position:fixed;inset:0;z-index:60;background:rgba(12,14,18,.82);display:flex;align-items:center;justify-content:center;}',
      '.isx-lbmask{position:absolute;inset:0;}',
      '.isx-lbwrap{position:relative;display:flex;flex-direction:column;align-items:center;gap:14px;max-width:92vw;}',
      '.isx-lbimg{max-width:100%;max-height:78vh;border-radius:8px;',
      'box-shadow:var(--dsw-elevation-prominent,0 18px 60px rgba(0,0,0,.35));}',
      '.isx-lbedit{display:inline-flex;align-items:center;gap:6px;border:0;border-radius:999px;padding:9px 20px;',
      'font:inherit;font-size:13px;font-weight:600;cursor:pointer;color:#fff;',
      'background:var(--dsw-alias-button-info-fill,rgb(65,118,230));box-shadow:0 4px 16px rgba(0,0,0,.4);}',
      '.isx-lbedit:hover{filter:brightness(1.12);}',
      '.isx-lbclose{position:absolute;top:14px;right:14px;width:34px;height:34px;border-radius:50%;border:0;cursor:pointer;',
      'background:rgba(255,255,255,.14);color:#fff;font-size:15px;line-height:1;}',
      '.isx-lbclose:hover{background:rgba(255,255,255,.28);}',
    ].join('');
    ...
    module.exports = {
      ...
      apply(ctx) {
        ...
        const ui = createStore({ open: false, mode: 'txt2img' });
        ...
        function EditorView(props) {
          ...
          // Wheel over the stage zooms with the cursor point kept steady. A
          // native listener: React wheel events are passive and cannot
          // preventDefault the viewport scroll.
          React.useEffect(() => {
            const vp = viewportRef.current;
            if (!vp || !dims) return;
            const onWheel = (event) => {
              ...
              const next = clamp(Math.round(zoomRef.current * factor), 20, 800);
              if (next === zoomRef.current) return;
              const box = vp.getBoundingClientRect();
              const cx = event.clientX - box.left;
              const cy = event.clientY - box.top;
              const sx = vp.scrollLeft + cx;
              const sy = vp.scrollTop + cy;
              const ratio = next / zoomRef.current;
              setZoom(next);
              // Re-anchor after the canvas re-layouts at the new zoom.
              requestAnimationFrame(() => {
                vp.scrollLeft = sx * ratio - cx;
                vp.scrollTop = sy * ratio - cy;
              });
            };
          ...
          // Grab cursor shows exactly while the canvas outgrows the viewport.
          React.useEffect(() => {
            const vp = viewportRef.current;
            if (!vp) return;
            setPannable(vp.scrollWidth > vp.clientWidth + 1 || vp.scrollHeight > vp.clientHeight + 1);
          }, [current, dims, zoom, fitScale]);
          ...
          // Drag-to-pan: once the canvas outgrows the viewport, pressing and
          // moving pans by scrolling. Crop mode keeps its own pointer logic.
          const onPanDown = (event) => {
            if (cropMode || event.button !== 0) return;
            const vp = viewportRef.current;
            if (!vp || (vp.scrollWidth <= vp.clientWidth && vp.scrollHeight <= vp.clientHeight)) return;
            event.preventDefault();
            panRef.current = { x: event.clientX, y: event.clientY, sl: vp.scrollLeft, st: vp.scrollTop };
            setPanning(true);
          };
          const onPanMove = (event) => {
            ...
            const vp = viewportRef.current;
            if (!pan || !vp) return;
            vp.scrollLeft = pan.sl - (event.clientX - pan.x);
            vp.scrollTop = pan.st - (event.clientY - pan.y);
          };
          const onPanEnd = () => {
          ...
          return ce('div', { className: 'isx-view' },
            ce('div', { className: 'isx-stage' },
              ...
              ce('div', {
                ref: viewportRef,
                className: 'isx-viewport' + (cropMode ? '' : panning ? ' isx-panning' : pannable ? ' isx-pannable' : ''),
                onMouseDown: onPanDown, onMouseMove: onPanMove, onMouseUp: onPanEnd, onMouseLeave: onPanEnd,
              },
                ce('div', {
                  ref: wrapRef,
        ...
        function WorkbenchBody() {
          useLocaleTick();
          // Mode lives in the shared store so entry points outside the
          // overlay (the message-image preview) can open the workbench
          // directly on the editor tab.
          const mode = useStoreValue(ui).mode;
          const setMode = (next) => ui.set({ mode: next });
          const tabs = [
        ...
        /** One gallery thumbnail: resolves its session-authorized URL and
         * reports clicks upward with the resolved source. */
        function PreviewThumb(props) {
          const spec = props.spec;
          const isPreview = 'preview' in spec;
          const [url, setUrl] = React.useState(() => {
            if (isPreview) return spec.preview.url;
            return props.loadImage.peek ? props.loadImage.peek(spec.attachment) : undefined;
          });
          const [failed, setFailed] = React.useState(false);
          const load = React.useCallback(() => {
            if (isPreview || url !== undefined || failed) return undefined;
            let alive = true;
            Promise.resolve(props.loadImage(spec.attachment))
              .then((resolved) => { if (alive) setUrl(resolved); })
              .catch(() => { if (alive) setFailed(true); });
            return () => { alive = false; };
          }, [isPreview, url, failed, props.loadImage, spec.attachment]);
          React.useEffect(load, [load]);
          const dims = isPreview ? spec.preview : spec.attachment;
          const name = (spec.attachment && spec.attachment.name) || (isPreview ? spec.preview.name : undefined) || t('imgAlt');
          if (url === undefined) {
            return failed
              ? ce('button', { className: 'isx-thumbload', type: 'button', title: t('thumbRetry'), onClick: () => setFailed(false) }, t('thumbRetry'))
              : ce('span', { className: 'isx-thumbload' }, ce('div', { className: 'isx-spin', style: { width: '18px', height: '18px', borderWidth: '2px' } }));
          }
          return ce('button', {
            className: 'isx-thumbbtn', type: 'button', title: t('viewOriginal'),
            'aria-label': t('viewOriginal') + ': ' + name,
            onClick: () => props.onOpen(url, name),
          }, ce('img', { src: url, alt: name, style: props.single ? singleFitStyle(dims) : { width: '64px', height: '64px', objectFit: 'cover' } }));
        }

        /** Document-level preview overlay with the workbench edit hand-off.
         * Rendered through a body portal so transformed message ancestors
         * cannot trap the fixed backdrop. */
        function PreviewLightbox(props) {
          React.useEffect(() => {
          ...
          return ReactDOM.createPortal(
            ce('div', { className: 'isx-lb', role: 'dialog', 'aria-modal': 'true', 'aria-label': t('previewDialog') },
              ce('div', { className: 'isx-lbmask', onMouseDown: props.onClose }),
              ce('div', { className: 'isx-lbwrap' },
                ce('img', { className: 'isx-lbimg', src: props.src, alt: props.alt }),
                ce('button', {
                  className: 'isx-lbedit', type: 'button',
                  onClick: () => {
                    // blob:/data: sources keep the editor canvas clean, so the
                    // full edit pipeline (crop, filters, compression) works.
                    data.set({ editorImage: props.src });
                    ui.set({ open: true, mode: 'edit' });
                    props.onClose();
                  },
                }, svgIcon(ICON_EDIT, 14), t('previewEdit'))),
              ce('button', { className: 'isx-lbclose', type: 'button', 'aria-label': t('previewClose'), onClick: props.onClose }, '✕')),
            document.body);
        }

        /** The message-flow image group renderer: the shipped gallery with a
         * preview that hands images straight into the workbench editor. */
        function MessagePreviewGroup(props) {
          const images = Array.isArray(props.images) ? props.images : [];
          const [open, setOpen] = React.useState(null);
          const single = images.length === 1;
          return ce('div', {
            className: 'isx-preview',
            style: props.align === 'right' ? { justifyContent: 'flex-end' } : undefined,
          },
            images.map((spec, index) => ce(PreviewThumb, {
              key: index,
              spec,
            ...
            open !== null ? ce(PreviewLightbox, { src: open.src, alt: open.alt, onClose: () => setOpen(null) }) : null);
        }
        ...
        // Replace the shipped message-image gallery: the preview gains a
        // direct edit hand-off into the workbench (blob:/data: sources keep
        // the editor canvas clean). The contribution belongs to this fiber,
        // so stopping the plugin restores the shipped renderer.
        slots.inject('conversation.message.images', () => slots.register(
          { name: 'conversation.message.images' },
          (props) => ce(MessagePreviewGroup, props),
        ));
